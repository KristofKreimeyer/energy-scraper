...
export function CardActions({ offer, votes }: { offer: GroupedOffer; votes?: VoteTally }) {
  ...
  const toggle = (p: "alarm" | "report") => setPanel((cur) => (cur === p ? null : p));
  ...
  const chip = (active: boolean) =>
    `inline-flex items-center gap-1.5 h-8 px-2.5 rounded-md border text-[0.76rem] font-semibold cursor-pointer ${
  ...
  return (
    <div className="relative z-10 flex flex-col gap-2.5" onClick={(e) => e.stopPropagation()}>
      ...
      {panel === "alarm" && <AlarmButton offer={offer} embedded />}
      {panel === "report" && <ReportPriceButton offer={offer} embedded onClose={() => setPanel(null)} />}
    </div>
  );
}
